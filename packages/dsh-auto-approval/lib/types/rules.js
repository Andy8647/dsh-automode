 ...
 * - 其它命令返回 undefined
 */
export function matchSelfKill(command) {
...
export function selfKillDenyReason(hostPid) {
    return ('auto-approval: this call was denied by the auto-approval security policy because it would ' +
...
export function createDenyGuard(getConfig) {
    return (execution) => {
        ...
        if (text === undefined)
            return undefined;
        if (matchFirst(text, config.deny) !== undefined)
            return DENY_REASON;
